...
//        }
        ...
        if (carrierID.equalsIgnoreCase("")) {
            JsonArray stops = (JsonArray) shipmentJson.get("shipment").getAsJsonObject().get("identifiers");
            for (JsonElement position : stops) {
                if ("CARRIER_SCAC".equals(position.getAsJsonObject().get("type").getAsString())) {
                    carrierID = position.getAsJsonObject().get("value").getAsString();
