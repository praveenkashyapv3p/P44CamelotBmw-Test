    ...
    public ResponseEntity<String> shipmentDetailsFromP44(@RequestBody String shipmentJson) {
        try {
            ...
            for (JsonElement eventsTyp : events) {
                if (eventsTyp.getAsJsonObject().get("type").getAsString().equalsIgnoreCase("TRACKING_START") ||
                        eventsTyp.getAsJsonObject().get("type").getAsString().equalsIgnoreCase("TRACKING_INITIATE") ||
                        eventsTyp.getAsJsonObject().get("type").getAsString().equals("UNKNOWN")) {
                    trackingJson = true;
                }
